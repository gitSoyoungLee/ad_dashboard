...
본 프로젝트는 작성자의 **인턴십 당시 데이터 분석 업무를 수행하며 느꼈던 실제 현업의 불편함**을 기술적으로 해결하기 위해 개인적으로 기획하고 개발한 토이
프로젝트입니다.
...
개발 과정에서 마주한 문제들을 분석하고 해결한 과정을 정리했습니다. 자세한
내용은 [Wiki](https://github.com/gitSoyoungLee/ad_dashboard/wiki)에서 확인할 수 있습니다.

| 주제                            | 문제 상황                                        | 해결 방법                                    |
|-------------------------------|----------------------------------------------|------------------------------------------|
| N+1 쿼리 문제                     | 루프 내 개별 쿼리로 인한 API 응답 지연                     | IN절 배치 쿼리 + GROUP BY 집계로 전환              |
| Meta API URI 템플릿 충돌           | JSON 중괄호가 URI 변수로 해석되는 문제 + Content-Type 불일치 | 문자열 기반 URI 템플릿 + 커스텀 MessageConverter 등록 |
| JPQL 반환 타입 ClassCastException | 다중 컬럼 집계 쿼리의 Object[] 반환 타입 캐스팅 실패           | 단일 컬럼 쿼리로 분리하여 타입 안전성 확보                 |
